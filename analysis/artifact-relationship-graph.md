...
| 2: Control reconnaissance | full-blind: neutral source/scope first. correction-validation: legacy_reconnaissance.md; legacy_user_flows.xlsx; full status, prior reports/checklist/dispositions immediately (reviewer read-only) | migration_status.yaml: PM only, never the reviewer | stage-02-pass-NNN.md | full-blind only: legacy_reconnaissance.md; legacy_user_flows.xlsx; migration_status.yaml; prior reports/checklist/dispositions after saved complete Phase A. correction-validation: no delayed inputs or new Phase A |
