...
for (const f of fs.readdirSync(WAR + '/WEB-INF/classes').filter(f => /\.properties$/.test(f))) {
...
{ const s = fs.readFileSync(LEG + '/README.md', 'utf8'); const m = /\*\*\s*([^*\/\s]+)\s*\/\s*([^*\s]+)\s*\*\*/.exec(s); if (m) { add(m[2]); vals.add(m[1] + ' / ' + m[2]); vals.add(m[1] + '/' + m[2]); vals.add(m[1] + ':' + m[2]); } }
