...
> [!WARNING]
> **Stage 1 re-entry draft (BA-001-06, after Stage 2 pass 004 `findings`): corrected static analysis of the fixed baseline package, not yet independently re-verified**
>
> The whole WAR was enumerated (964 files). Stage 2 pass 004 returned 6 findings. All 6 were confirmed against the source and accepted, and F-003 and F-005 were extended by the sweeps. The main corrections are these:
> - **Login page (F-001):** the default-bundle login help text states the factory default administrator login pair, and that pair matches the seeded account (row 9). The value is not reproduced anywhere (CHK-009).
> - **Task board (F-002):** the board works; the link tag adds the parameter it needs (row 114, now `Yes`).
> - **Configuration (F-003):** the Hibernate and pool settings in the property files never reach the web session factory or the pool (row 68).
> - **Print layout (F-004):** it is reachable by URL (row 192).
> - **Unauthenticated exposure (F-005):** the live activity log is written into the public web root, and `index.jsp` runs without a security filter.
> - **Wiki rendering (F-006):** it makes a server-side HTTP request.
>
> The parity map now holds 210 scenario rows in 18 epics: 113 `Yes`, 77 `Inferred`, 19 `Partial` and 1 `No`. BA-001-06 added 2 rows, changed the content of 11 and renumbered references in 13. Dispositions are in [`analysis/stages/stage-01/stage-02-pass-004-dispositions.md`](./stages/stage-01/stage-02-pass-004-dispositions.md); the earlier records stay unchanged as history. All rows are static-only. The package is the project's fixed baseline, not an unmodified original; 31 rows carry per-conclusion provenance notes (GAP-005, Q1 decided). Fourteen gaps are recorded (GAP-001..GAP-014). Q2-Q4 are deferred to Stages 4 and 9. Insecure or broken legacy behavior (GAP-007, Q2 and Q3 facts) is not permission to carry it into the new application.
>
> **Next:** PM verifies RESULT BA-001-06, integrates status and launches a new fresh blind Stage 2 pass. The corrections are not yet independently verified. Decisions and open questions are in [Owner Decisions And Open Questions](#read-owner-decisions-and-open-questions).
>
...
- Snapshot date: WAR entry timestamps run from 2011-11-20 to 2011-12-04. The analysis was performed on 2026-09-23. The bounded correction BA-001-02 and the re-entry corrections BA-001-03 to BA-001-06 were made on 2026-09-24.
- Analyst: Business Analyst (role `ba`, mode `author`), tasks **BA-001-01** (first pass), **BA-001-02** (bounded correction applying the owner's Q1 decision), **BA-001-03** (Stage 1 re-entry after [`analysis/reviews/stage-02-pass-001.md`](./reviews/stage-02-pass-001.md)) **BA-001-04** (Stage 1 re-entry after [`analysis/reviews/stage-02-pass-002.md`](./reviews/stage-02-pass-002.md)) **BA-001-05** (Stage 1 re-entry after [`analysis/reviews/stage-02-pass-003.md`](./reviews/stage-02-pass-003.md)) and **BA-001-06** (Stage 1 re-entry after [`analysis/reviews/stage-02-pass-004.md`](./reviews/stage-02-pass-004.md)), all assigned by PM (task PM-001, Claude Code session `d0ec1166-ffc9-446a-ba86-d768242e6de8`).
...
|---|---|---|---|---|
| `sha256sum legacy/*` (before the work and again at the end) | revision `1e12347`, legacy tree `3bd350fa`; re-run in BA-001-04 on branch `stage-01/pass-002-corrections` from `main` `25b25c4` and in BA-001-05 on branch `stage-01/pass-003-corrections` from `main` `9a9f432` and in BA-001-06 on branch `stage-01/pass-004-corrections` from `main` `c43363d`, same legacy tree | passed: all four hashes equal the values in [Scope And Provenance](#read-scope-and-provenance) | command output in the task transcript; re-check command recorded here | Nothing about behavior. |
...
| `npm --prefix analysis/tools run audit:project` | same | passed (exit 0; re-run in BA-001-04, BA-001-05 and BA-001-06: `PROJECT CONFIG AUDIT OK` at stage-01) | audit output | Project configuration structure only. |
| `npm --prefix analysis/tools run audit:artifact-links` | same | passed (exit 0; re-run in BA-001-04, BA-001-05 and BA-001-06: 209 Markdown documents checked) | audit output | Link form only. |
| `node analysis/tools/artifact-reading.js --file analysis/legacy_reconnaissance.md` (and on the correction records [`analysis/stages/stage-01/stage-02-pass-002-dispositions.md`](./stages/stage-01/stage-02-pass-002-dispositions.md) in BA-001-04 and [`analysis/stages/stage-01/stage-02-pass-003-dispositions.md`](./stages/stage-01/stage-02-pass-003-dispositions.md) in BA-001-05, and on [`analysis/stages/stage-01/stage-02-pass-004-dispositions.md`](./stages/stage-01/stage-02-pass-004-dispositions.md) in BA-001-06) | same | passed: no errors in either file | tool output | Reading structure only. |
| `npm --prefix analysis/tools run audit:workbook:excel` | Excel Desktop (Office16) through COM, read-only open, run once | passed (exit 0): `EXCEL DESKTOP OPEN AUDIT OK`; workbook hash unchanged. Re-run once in BA-001-04 after the final workbook write: exit 0, hash `73a8e05b…0e11` before and after. Re-run once in BA-001-05 after the final workbook write: exit 0, hash `24630a19…670a` before and after. Re-run once in BA-001-06 after the final workbook write: exit 0, hash `fa0ea118…4d76` before and after | `.migration-tmp/stage-01/out/audit-excel.txt`, `out/audit-excel-4.txt`, `out/audit-excel-5.txt`, `out/audit-excel-6.txt` | A clean open does not prove content or visual layout. |
...
| Mobile WAP pages | Links hard-code `/xplanner`; web filter may intercept mobile login; DispatchForward requires a projectId the links do not pass (`WAR:WEB-INF/mobile-struts-config.xml:20-49`) | 232-234 | GAP-008, GAP-014 |
| JAX-WS endpoint | Servlet commented out, class absent (`WAR:WEB-INF/sun-jaxws.xml:3`) | 219 | GAP-003 |
| File manager | Reachable only by URL (`WAR:WEB-INF/struts-config.xml:401-403`) | 176 | none |
...
- Row statuses, counted from the workbook after BA-001-06: 113 `Yes`, 77 `Inferred`, 19 `Partial`, 1 `No` (210 in total).
...
- [x] The parity workbook is populated and linked to concrete evidence (210 rows after BA-001-06).
...
- **Self-check, BA-001-06 (current):** Stage 1 re-entry after [`analysis/reviews/stage-02-pass-004.md`](./reviews/stage-02-pass-004.md). The scope is both Stage 1 records and the correction record [`analysis/stages/stage-01/stage-02-pass-004-dispositions.md`](./stages/stage-01/stage-02-pass-004-dispositions.md), at the versions given in RESULT BA-001-06.
