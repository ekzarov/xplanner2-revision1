...
> [!WARNING]
> **Stage 1 re-entry draft (BA-001-07, impact-scoped correction after Stage 2 pass 005 `findings`): corrected static analysis of the fixed baseline package, not yet independently re-verified**
>
> The whole WAR was enumerated (964 files). Stage 2 pass 005 returned 5 findings. BA-001-07 corrected them within the boundary PM assigned, together with the occurrences found by three mechanism checks: (a) wiring and injected properties, (b) request-value flow, (c) read-path side effects. All 5 findings were confirmed; F-005 was narrowed. The main corrections are these:
> - **Wiring:** the people import (rows 50-51), the file manager (row 176) and the WAP actions (rows 232, 234) run with dependencies that are never injected.
> - **Request values:** they reach HQL in the aggregate timesheet (row 165) and the note list (row 169). Raw request parameters are written unescaped into the task board script, into 17 edit and confirmation pages and into the not-found page. The error page lists every request parameter, and the remember-me cookies live about 68 years without a Secure flag.
> - **Read-path writes:** the first attachment creates directory rows (row 172); the file-manager listing fails before its write is reached.
>
> The parity map holds 210 scenario rows in 18 epics: 111 `Yes`, 77 `Inferred`, 21 `Partial` and 1 `No`. BA-001-07 added no rows and changed 31. The correction record is [`analysis/stages/stage-01/stage-02-pass-005-dispositions.md`](./stages/stage-01/stage-02-pass-005-dispositions.md). All rows are static-only. The package is the project's fixed baseline, not an unmodified original; 31 rows carry per-conclusion provenance notes (GAP-005, Q1 decided). Fourteen gaps are recorded (GAP-001..GAP-014). Q2-Q4 are deferred to Stages 4 and 9. Insecure or broken legacy behavior (GAP-007, Q2 and Q3 facts) is not permission to carry it into the new application.
>
> **Next:** PM verifies RESULT BA-001-07, integrates status and launches a new fresh blind Stage 2 pass. The corrections are not yet independently verified. Decisions and open questions are in [Owner Decisions And Open Questions](#read-owner-decisions-and-open-questions).
>
...
- Snapshot date: WAR entry timestamps run from 2011-11-20 to 2011-12-04. The analysis was performed on 2026-09-23. The bounded correction BA-001-02 and the re-entry corrections BA-001-03 to BA-001-06 were made on 2026-09-24, and the impact-scoped correction BA-001-07 on 2026-09-25.
- Analyst: Business Analyst (role `ba`, mode `author`), tasks **BA-001-01** (first pass), **BA-001-02** (bounded correction applying the owner's Q1 decision), **BA-001-03** (Stage 1 re-entry after [`analysis/reviews/stage-02-pass-001.md`](./reviews/stage-02-pass-001.md)) **BA-001-04** (Stage 1 re-entry after [`analysis/reviews/stage-02-pass-002.md`](./reviews/stage-02-pass-002.md)) **BA-001-05** (Stage 1 re-entry after [`analysis/reviews/stage-02-pass-003.md`](./reviews/stage-02-pass-003.md)) **BA-001-06** (Stage 1 re-entry after [`analysis/reviews/stage-02-pass-004.md`](./reviews/stage-02-pass-004.md)) and **BA-001-07** (impact-scoped correction after [`analysis/reviews/stage-02-pass-005.md`](./reviews/stage-02-pass-005.md)), all assigned by PM (task PM-001, Claude Code session `d0ec1166-ffc9-446a-ba86-d768242e6de8`).
...
| Tiles | `WAR:WEB-INF/tiles-definitions.xml`, `tiles-pages.xml` | 5 definitions (`tiles:default`, `tiles:view`, `tiles:print`, `tiles:edit`, `viewLayout`). `tiles:print` is used by `ContentTag#doStartTag` whenever the request has a `print` parameter (`PrintLinkTag#isInPrintMode`). Every page built with `xplanner:content` (43 JSPs) can therefore be shown without header and footer by URL, and no page links to it. This was corrected in BA-001-06 (pass-004 F-004; row 192). | Inspected fully. |
| Spring 3.0.5 context | `WAR:WEB-INF/classes/spring-beans.xml` (imports `spring-caching.xml`, `spring-dao.xml`, `spring-security.xml`), `spring-web.xml` | 58 + 5 + 13 + 14 + 3 top-level beans. Liquibase runs before the session factory. Annotation session factory scans `net.sf.xplanner.domain` and loads only `mappings/Metrics.xml`. One active scheduled task (`missingTimeEntryNotifier`, cron `0 5 0 * * *`). The Quartz data-sampling trigger is defined but its scheduler is commented out. Component scans: `net.sf.xplanner.rest` and `net.sf.xplanner.web`. **Loaders (BA-001-05, F-002):** **Injected properties (BA-001-07, pass-005 mechanism (a)):** `.migration-tmp/stage-01/tools/wiring-07.js` matched every setter of the 80 action beans, the 34 autowired root beans and the 7 bean-less Struts mappings against beans of that name or type. Of 461 properties, 43 have no bean of that name or type; one more reference resolves through the class-name alias that Spring registers. Nine of the 43 are needed on the entry path and stay null: `personDao` of `/import/people`, `fileSystem` of `/view/directory`, `authenticator` of `/mobile/login`, and `authorizer` of the six WAP views. The other 34 fall into these groups: unused on the path; constructor defaults (`DispatchForward` authorization flag); request fallback (`type` of `/view/history`); set at runtime by their callers; resolved by the framework (aware interfaces, application context); or used only behind a disabled flag (`dataSampler.hibernateOperations`) (`out/wiring-07.txt`, `out/wiring-impact-07.txt`). `spring-beans.xml` is loaded twice. The root `ContextLoaderListener` loads it (`WAR:WEB-INF/web.xml:32-38,194-196`), and so does the Struts `ContextLoaderPlugIn`, together with `action-servlet.xml` and `test-action-servlet.xml` (`WAR:WEB-INF/struts-config.xml:438-441`). Every singleton is therefore expected twice: the Liquibase run, the scheduler with the reminder job, the connection pool, the session factory and the authorizer initializer. The action beans are wired from the plug-in context (`Inferred`, row 65). A third loader, `MainBeanFactory` (an `XmlBeanFactory` of `/spring-beans.xml`), has no caller. `spring-web.xml` is loaded only by the Spring `DispatcherServlet` (`web.xml:264-272`). Both the Struts Tiles plug-in and `spring-web.xml` read `tiles-definitions.xml`, each for its own container. The loader list for all 55 configuration files is `.migration-tmp/stage-01/out/loaders-05.txt`. | Inspected fully. Provenance: `spring-beans.xml` is one of the 3 rewritten ZIP entries and carries the P-02 patch; its other wiring is not guaranteed upstream (GAP-005 P-02, P-05). |
...
<br>- Unmapped paths: `task.story` in the two iCal queries (row 228) and in two jrpdf data sources (rows 211, 213); the undeclared alias `object` and the missing `projectId` in SOAP `getCurrentIteration` (row 220); `a.targetId`/`a.name` in SOAP `deleteAttribute` (row 221).
...
| Configuration | `WAR:WEB-INF/classes/xplanner.properties` (317 lines) overridden by `xplanner-custom.properties` (load order per `XPlannerProperties$PropertyInitializer`); 7 unused variants `xplanner-custom-*.properties`; `log4j-war.xml`; `ehcache.xml`; `spy.properties` | Effective settings: the connection pool uses the MySQL driver, URL `jdbc:mysql://db/xplanner` and the configured account (`spring-beans.xml:60-69`). The `hibernate.*` settings of the property files (dialect `XPlannerMySQLDialect`, `show_sql`, second-level and query cache, query substitutions) and all `hibernate.dbcp.*` pool settings are not passed to the web session factory or the pool (`spring-beans.xml:77-81`; no `hibernate.properties` ships). They are therefore expected to have no effect there (`Inferred`, row 68). This was corrected in BA-001-06 (pass-004 F-003). The consumer sweep over all 103 distinct keys found 42 keys with no consumer by name, among them the cache, pool, Quartz and `actionbuttons.*` keys (`.migration-tmp/stage-01/out/config-consumers-06.txt`). The application URL is `http://localhost:8080/xplanner`; SMTP localhost:25; export formats `xml,mpx,mspdi,pdf,jrpdf`; statistics on; velocity chart hidden; progress bar html; default story priority 4; content search global. The database user and password values are present but not reproduced. p6spy is not wired. | Both effective files were read fully. The unused variants, `ehcache.xml` and `spy.properties` were not read in detail. Provenance: both effective properties files are rewritten entries; database values are the known patch (P-01); the other values cannot be separated (P-03, P-04). |
...
| Mobile channel | `mobile-struts-config.xml`, `WAR:WEB-INF/jsp/wap/*.jsp` | 7 WML read-only pages. Links are hard-coded to `/xplanner/...`. The 7 mappings have no Spring bean, so Struts creates the actions without injection. The WAP login is therefore expected to fail on the missing authenticator, and a view with a `projectId` on the missing authorizer (BA-001-07, pass-005 F-004; rows 232, 234). `wap/auth.jsp` is unreferenced debug markup that echoes credentials; it is excluded as unreachable. | Inspected fully. |
...
| Mobile WAP users | `/do/mobile/*` (WML) | same, plus a WML browser | blocked (partial) | Links are hard-coded to `/xplanner`. The web filter may intercept mobile login (UF-018). The actions are Struts-created without injection, so login and views with a `projectId` are expected to fail on null dependencies (BA-001-07, pass-005 F-004). |
| SOAP API clients | `/soap/XPlanner` with HTTP Basic, and `/servlet/AxisServlet` (unfiltered) | Axis 1.4 in the web app | inferred | `WAR:WEB-INF/server-config.wsdd`, `com.technoetic.xplanner.soap.XPlanner`. `getCurrentIteration` and `deleteAttribute` are expected to fail on unmapped query paths (BA-001-05, F-001; rows 220-221). |
...
| Calendar clients | `/ical/<userId>.ics` with HTTP Basic | web app | blocked (partial) | `com.technoetic.xplanner.ical.iCalServlet`. Both queries use `task.story`, which `Task` does not map, so an error response is expected instead of a calendar (BA-001-05, F-001; row 228). |
| Chart images | `/cewolf/*` rendered on demand from JSP chart tags; the servlet has no security filter (BA-001-06 sweep), and the chart data is produced by the JSP of a signed-in page | JFreeChart. A headless or X environment is needed per the `xplanner.statistics` comment. | inferred | `WAR:WEB-INF/web.xml:211-225`, `iterationStatistics.jsp`, `timesheet.jsp` |
| Daily missing-time-entry job (operator/batch) | Spring `task:scheduled`, cron `0 5 0 * * *`, inside the web app; registered in both Spring contexts that load `spring-beans.xml`, so it is expected to run twice (BA-001-05, F-002) | SMTP, database | inferred | `spring-beans.xml:362-367`, `web.xml:32-38`, `struts-config.xml:438-441`; rows 65, 200-201 |
...
| Activity log (operator) | `ActivityLogFilter` on `/do/*` writes `${xplanner-plus.root}/xplanner-plus-activity.log` into the public web root, where no security filter applies (BA-001-06, pass-004 F-005; row 73) | writable web-app root | inferred | `WAR:WEB-INF/classes/log4j-war.xml:44-58`, `WAR:WEB-INF/web.xml:45-52` |
...
| Other web actions (export, import, move/continue, reorder, time, notes, attachments, file manager, integration queue, iteration start/close/continue, notification receivers, role editor, timesheet, ID search, test/admin utilities, "me" page) | none as a gate | `out/authtrace.json` | 30, 35, 164, 184 |
...
| DispatchForward views with authorization required (default true): `/do/view/integrations` and the 7 WAP actions | projectId parameter and `system.project` read, else not-authorized forward | `DispatchForward#<init>/#isSecure/#execute` | 28, 234 |
| Content search, aggregate timesheet | result filtering by read permission | `SearchResultAuthorizationPredicate#isResultReadableByUser`, `AggregateTimesheetQuery#getTimesheet` | 167, 180 |
| SOAP | per-object read, create, edit and delete checks; the 5 attribute operations are unchecked | `XPlanner#hasPermission` via `#getObject`, `#selectAccessibleObjects`, `#addObject`, `#updateObject`, `#removeObject` | 218 |
| REST | none, and no filter | `ViewService`, `UpdateService` | 225 |
| iCal | another person's calendar needs `admin.edit` on `system.person` | `iCalServlet#doGet` | 230 |
| Descriptor URL role constraints | never evaluated | `SecurityConfiguration#isAuthorized` has no caller | 32, 233 |

**Unauthenticated surfaces, listed mechanically (BA-001-06, pass-004 P-2).** `.migration-tmp/stage-01/tools/unauth-06.js` compared every servlet mapping and the welcome file with the url-patterns of the three security filters (`WAR:WEB-INF/web.xml:155-176`). It also listed the files shipped in, or written into, the web root and checked every bundle text used on the public pages in all 10 variants. Output: `out/unauth-06.txt`.

- **Unfiltered entry points:** `/rest/*` (row 225), `/servlet/AxisServlet` (row 216), `/cewolf/*` and the welcome file `index.jsp` (row 18).
- **Shipped in the web root:** 111 files. They include 2 JSPs (`index.jsp`, `calendar/calendar-i18n.jsp`), the 2011 activity log (GAP-013) and 2 import templates.
- **Written into the web root at runtime:** the live activity log (row 73). The `FILE` appender (`xplanner-plus.log`) is not referenced by any logger.
- **Public-page texts:** 41 bundle keys. Only `login.instructions` in the default bundle carries a credential (row 9). Its value is not reproduced (CHK-009).

**Additional defects found by the sweep (not reviewer findings):**

- **AF-01:** `ViewIterationMetricsAction#getRepository` returns null, so the iteration metrics are expected to be empty (rows 107-108, now `Partial`).
...
- **The row cites the server-side check:** rows 17, 28, 31, 33, 167, 180, 218, 221, 228, 230 and 234.
...
- **Display-level only, no server-side check:** rows 24, 26, 41-43, 46, 48-50, 59, 76, 78, 81, 82, 85, 88, 90, 91, 95, 97, 98, 101, 115, 121, 125, 126, 129, 134, 139-142, 144, 149, 169, 173, 174 and 196.
...
| External wiki | TWiki-style links through `GenericWikiAdapter` (URL `localhost:9090/vqw` in the shipped configuration). Rendering a WikiWord makes a server-side HTTP request to the configured topic URL (`GenericWikiAdapter#isTopicExisting`, `URL.openStream` without timeouts). Existing topics are cached; failures are not, so the word is requested again on every rendering (BA-001-06, pass-004 F-006; row 189). | `xplanner.properties:92-127`, `:113-116` | not checked | Depends on the configured URL being reachable; an unreachable host is expected to delay page rendering (runtime unverified; P-03). The outbound-call sweep found only this request and the e-mail stylesheet fetch (`out/render-side-effects-06.txt`). |
| Twitter and Facebook | Share links on view pages: a Twitter status link and a Facebook feed-dialog link. The LinkedIn link, the `connect.facebook.net` script and the Facebook like widget sit inside an HTML comment and are not rendered (corrected in BA-001-04, F-003). | `WAR:WEB-INF/jsp/layout/viewLayout.jsp:73-88` (live links), `:92-95` (commented block); the same commented block is in `WAR:WEB-INF/jsp/layout/viewLayoutNew.jsp:114-117` | not checked | The links go to external services and are opened by the browser. No external script is loaded. Row 193. |
...
|---|---|---|---|---|
| `sha256sum legacy/*` (before the work and again at the end) | revision `1e12347`, legacy tree `3bd350fa`; re-run in BA-001-04 on branch `stage-01/pass-002-corrections` from `main` `25b25c4` and in BA-001-05 on branch `stage-01/pass-003-corrections` from `main` `9a9f432` in BA-001-06 on branch `stage-01/pass-004-corrections` from `main` `c43363d` and in BA-001-07 on branch `stage-01/pass-005-corrections` from `main` `ca517e5`, same legacy tree | passed: all four hashes equal the values in [Scope And Provenance](#read-scope-and-provenance) | command output in the task transcript; re-check command recorded here | Nothing about behavior. |
...
| `node .migration-tmp/stage-01/tools/edit-ba-001-05-rows.js` and `remap-rows-05.js` (BA-001-05, each run once) | row data and this record | passed: 6 rows inserted after rows 51, 63, 107, 120, 127 and 141; existing row references in 10 cells and 68 lines of this record remapped | `out/row-map.txt` | Mechanical placement only; content is checked by CHK-001 and by the sweeps. |
| `node .migration-tmp/stage-01/tools/unauth-06.js` (BA-001-06) | read-only; comment-blanked `web.xml` and `log4j-war.xml`; web-root file list; public-page bundle keys in all 10 variants | passed: 3 unfiltered servlet mappings plus the unfiltered welcome file; 111 shipped web-root files; 1 live log appender writing into the web root; 41 public-page keys, of which 1 carries a credential | `out/unauth-06.txt` (keys and line numbers only, no values) | Static serving by the container is not observed. |
| `node .migration-tmp/stage-01/tools/config-consumers-06.js` (BA-001-06) | read-only; keys of both effective property files against class strings, string constants, JSP text and Spring placeholders | passed: 111 keys (103 distinct), 42 without a consumer by name | `out/config-consumers-06.txt` (keys only) | Keys built at runtime from fragments could be missed; every hit was classified by hand. |
| `node .migration-tmp/stage-01/tools/render-side-effects-06.js` (BA-001-06) | read-only over `out/classes.json` | passed: outbound calls only in `GenericWikiAdapter#isTopicExisting` and the e-mail stylesheet fetch; `print` is the only request parameter that switches the layout | `out/render-side-effects-06.txt` | Calls made inside library code are not traced. |
| `node .migration-tmp/stage-01/tools/edit-ba-001-06-rows.js`, `-rows-b.js` and `remap-rows-06.js` (BA-001-06, each run once) | row data and this record | passed: 2 rows inserted after rows 67 and 190; row references in 13 cells and 77 lines of this record remapped | `out/row-map.txt` | Mechanical placement only; content is checked by CHK-001 and the sweeps. |
| `node .migration-tmp/stage-01/tools/chk009-scan-06.js` (BA-001-06) | read-only; credential values are extracted in memory from the WAR sources, [`legacy/docker-compose.yml`](../legacy/docker-compose.yml) and [`legacy/demo-seed.sql`](../legacy/demo-seed.sql), then searched for in the changed records, the new record and the new scratch outputs | passed: 0 hits; the scan prints file names and counts only | `out/chk009-06.txt` | A value that was changed or encoded would not be found. |
| `node .migration-tmp/stage-01/tools/wiring-07.js` and `wiring-impact-07.js` (BA-001-07, mechanism (a)) | read-only; Spring contexts parsed comment-blanked; setters of the application classes; static call graph of each action's `execute` path | passed: 121 handlers (80 action beans, 34 autowired root beans, 7 bean-less Struts mappings), 461 injected properties; 43 without a bean, 9 of them needed on the entry path | `out/wiring-07.txt`, `out/wiring-impact-07.txt` | Spring behavior for aware interfaces, resolvable dependencies and the class-name alias is framework knowledge (GAP-011). |
| `node .migration-tmp/stage-01/tools/query-sinks-07.js` and `request-echo-07.js` (BA-001-07, mechanism (b)) | read-only; bytecode listings of query-building methods; comment-blanked JSPs | passed: 77 query-executing methods, 47 building text at runtime, 16 appending non-constant text; 2 JSP where clauses built from request values; 71 request-value hits in JSPs, classified into reflected, numeric, bound and unreachable | `out/query-sinks-07.txt`, `out/request-echo-07.txt`, `out/reflections-07.txt` | Framework escaping (Struts tags, EL) is taken from the tag contracts, not observed. |
| `node .migration-tmp/stage-01/tools/persist-trace-07.js` (BA-001-07, mechanism (c)) | read-only static call graph from 125 read entry points (41 web, 6 WAP, 20 SOAP `get*`, 3 REST, 1 iCal, 2 Spring MVC, 52 tag handler methods) to persistence calls | passed: 1 read entry point (the file manager) reaches a write, and it is expected to fail before it | `out/persist-trace-07.txt` | Control flow is not followed; the session filter commits nothing (no flush or commit in `HibernateSessionFilter#doFilter`). |
| `node .migration-tmp/stage-01/tools/edit-ba-001-07-rows.js`, `edit-ba-001-07-recon.js` and `-recon-b.js` (BA-001-07, each run once) | row data and this record | passed: 31 rows changed, none added; each edit asserts a unique requirement prefix | `out/row-map.txt` | Mechanical placement only; content is checked by CHK-001 and the sweeps. |
...
| `node .migration-tmp/stage-01/tools/build-workbook.js` | exceljs from the project-local analysis tool dependencies; governed `setFill`, `reconcileWorksheetOutlineHierarchy`, `writeWorkbookFile` | passed (BA-001-07 final build): 18 epics, 210 scenario rows, last row 234, outline reconciled, header rows 1-6 unchanged, 31 provenance notes applied | [`analysis/legacy_user_flows.xlsx`](./legacy_user_flows.xlsx) | Writing the rows does not prove them correct. Stage 2 must verify them independently. |
| `npm --prefix analysis/tools run sync:workbook-progress` | same | passed: 18 epic banners updated to `(0%)`; overall 0 / 210 | workbook row 3 and banners | Progress is 0% by definition at Stage 1. |
| `npm --prefix analysis/tools run audit:workbook` | same | passed: `WORKBOOK AUDIT OK` (Scenarios 210; open 210; epics 18; revision sheets 0) | audit output | Structure and colour rules only, not semantic correctness. |
| `npm --prefix analysis/tools run audit:project` | same | passed (exit 0; re-run in BA-001-04 to BA-001-07: `PROJECT CONFIG AUDIT OK` at stage-01) | audit output | Project configuration structure only. |
| `npm --prefix analysis/tools run audit:artifact-links` | same | passed (exit 0; re-run in BA-001-04 to BA-001-07: 216 Markdown documents checked) | audit output | Link form only. |
| `node analysis/tools/artifact-reading.js --file analysis/legacy_reconnaissance.md` (and on the correction records [`analysis/stages/stage-01/stage-02-pass-002-dispositions.md`](./stages/stage-01/stage-02-pass-002-dispositions.md) in BA-001-04 and [`analysis/stages/stage-01/stage-02-pass-003-dispositions.md`](./stages/stage-01/stage-02-pass-003-dispositions.md) in BA-001-05, and on [`analysis/stages/stage-01/stage-02-pass-004-dispositions.md`](./stages/stage-01/stage-02-pass-004-dispositions.md) in BA-001-06, and on [`analysis/stages/stage-01/stage-02-pass-005-dispositions.md`](./stages/stage-01/stage-02-pass-005-dispositions.md) in BA-001-07) | same | passed: no errors in either file | tool output | Reading structure only. |
| `npm --prefix analysis/tools run audit:workbook:excel` | Excel Desktop (Office16) through COM, read-only open, run once | passed (exit 0): `EXCEL DESKTOP OPEN AUDIT OK`; workbook hash unchanged. Re-run once in BA-001-04 after the final workbook write: exit 0, hash `73a8e05b…0e11` before and after. Re-run once in BA-001-05 after the final workbook write: exit 0, hash `24630a19…670a` before and after. Re-run once in BA-001-06 after the final workbook write: exit 0, hash `fa0ea118…4d76` before and after. Re-run once in BA-001-07 after the final workbook write: exit 0, hash `a87c8383…6f99` before and after | `.migration-tmp/stage-01/out/audit-excel.txt`, `out/audit-excel-4.txt`, `out/audit-excel-5.txt`, `out/audit-excel-6.txt`, `out/audit-excel-7.txt` | A clean open does not prove content or visual layout. |
...
| GAP-011 | The 102 library JARs were not analysed beyond entry names. Framework behavior (Struts, Spring, Hibernate, Axis, Jersey, displaytag) is assumed standard for the bundled versions. | Framework defaults such as validation, paging, error handling and message fallback are inferred. Examples: which locale a message lookup without a locale uses (the stored task type label, row 135), and the fallback to the default bundle for keys that a bundle lacks (CHK-004 notes). Further examples (BA-001-05):
<br>- whether Hibernate passes unqualified column names in HQL (`userid` in the login query, row 11; `attachedTo_Id` in SOAP `getNotesForObject`, row 220) to SQL unchanged;
<br>- that an unresolvable dotted property path fails the query (rows 211, 213, 220, 221, 228);
<br>- how a child Spring context that redefines the parent's beans behaves (row 65). | Stage 3 observation. | BA | open |
| GAP-012 | The people import page offers `files/peopleImportTemplate.xls`, while `ImportPeopleAction` parses comma-separated text lines. In addition, the import cannot save any person, because `personDao` is never injected (BA-001-07, pass-005 F-001; rows 50-51). | Behavior with the offered template is unclear. With either format, the first valid line is expected to fail on save. | Stage 3 test, then Stage 4 disposition. | BA, owner | open (QUESTION) Q4 is deferred to Stage 4 (`legacy-open-questions-q2-q4:xplanner2-revision1`). |
| GAP-014 | Effects found only statically during the BA-001-03 and BA-001-05 sweeps. BA-001-03: AF-01 (iteration metrics repository is null), AF-02 (role editor save hook never called), the WAP DispatchForward projectId requirement and the jQuery/locale date-picker conflict. BA-001-05: queries on property paths that the entities do not map (iCal feed, SOAP `getCurrentIteration` and `deleteAttribute`, jrpdf data sources; pass-003 F-001), and the double loading of `spring-beans.xml` (duplicate reminder job and Liquibase run; pass-003 F-002). BA-001-07: dependencies that are never injected (people import, file manager, WAP actions; pass-005 F-001, F-004), request values in HQL and unescaped output (pass-005 F-002, F-003), and directory rows created on first use (pass-005 F-005). They rest on bytecode listings, call graphs and descriptor reads, not on runtime. | Rows 35, 65, 71, 107-108, 200-201, 211, 213, 220, 221, 228 and 234 carry these conclusions as `Partial` or `Inferred`, and so do rows 50-51, 176, 232 and 234 for BA-001-07. | Stage 3 confirmation with the affected roles and locales; for BA-001-07, a people import, a file-manager request and a WAP login; for BA-001-05, an iCal request, the two SOAP calls and a count of reminder e-mails and Liquibase runs per startup. | BA (Stage 3) | open. [`analysis/stages/stage-01/stage-02-pass-001-dispositions.md`](./stages/stage-01/stage-02-pass-001-dispositions.md); [`analysis/stages/stage-01/stage-02-pass-003-dispositions.md`](./stages/stage-01/stage-02-pass-003-dispositions.md) |
| GAP-013 | `xplanner-plus-activity.log` at the WAR root is a runtime artifact from the original build machine. It contains user IDs and IP addresses from 2011-12-03. At runtime the application appends its live activity log to the same web-root path (BA-001-06, pass-004 F-005; row 73). | Excluded as evidence. The shipped file, and the live log that replaces it, are expected to be served statically to anyone if the container serves web-root files (framework behavior, GAP-011). | Owner or Architect note for the target (do not ship runtime logs). | owner via PM | recorded |
...
| P-03 | All non-database values read from `xplanner.properties`: login modules, error filing text, progress bar, chart display, story import headers, default priority, twiki schemes, integration listeners, SMTP defaults, search scope, auto-extend, export formats. | A rewritten entry with no content sign of change for these keys. It is not guaranteed upstream. | Rows 9, 11, 21, 56, 67, 94, 109, 110 (progress chart flag, BA-001-05), 110, 114, 117, 121, 177, 188, 196, 201, 203, 206 | Whether each of these values is the upstream default. | The upstream `xplanner.properties`. Baseline behavior is not blocked. |
...
|---|---|---|---|
| Story features (view, edit, delete; project and iteration lists) | Action type `net.sf.xplanner.domain.Feature` is absent; `com.technoetic.xplanner.domain.Feature` is unmapped; links are commented out (`WAR:WEB-INF/action-servlet.xml:96-100,202-212`; `WAR:WEB-INF/jsp/view/iteration/links.jsp:32-35`) | 194 | GAP-003, GAP-004 |
| Continuous integration queue | Entity class unmapped; no live link (`WAR:WEB-INF/struts-config.xml:388-394`) | 196-198 | GAP-004 |
| Settings pages | Editor JSP absent; list page unlinked (`WAR:WEB-INF/struts-config.xml:405,419-421`) | 59-60 | GAP-003 |
| JasperReports (jrpdf) exports | Templates `JR*.jrxml` absent (class `PdfReportExporter`) | 211, 213 | GAP-003 |
| Tabbed iteration view | `view/iterationTabs.jsp` absent (`WAR:WEB-INF/struts-config.xml:199-201`) | 105 | GAP-003 |
| Not-authorized page | `security/notAuthorized.jsp` absent (`WAR:WEB-INF/struts-config.xml:139`) | 28 | GAP-003 |
| Mobile WAP pages | Links hard-code `/xplanner`; web filter may intercept mobile login; DispatchForward requires a projectId the links do not pass; the Struts-created actions have no `authenticator` or `authorizer` (`WAR:WEB-INF/mobile-struts-config.xml:20-49`) | 232-234 | GAP-008, GAP-014 |
| JAX-WS endpoint | Servlet commented out, class absent (`WAR:WEB-INF/sun-jaxws.xml:3`) | 219 | GAP-003 |
| File manager | Reachable only by URL (`WAR:WEB-INF/struts-config.xml:401-403`). Every operation is expected to fail, because the action's `fileSystem` is never injected (`WAR:WEB-INF/action-servlet.xml:396`; the bean is named `virtualFileSystem`, `spring-beans.xml:263`) | 176 | GAP-014 |
| People import | Line checks run, but saving is expected to fail, because `personDao` is never injected (`WAR:WEB-INF/action-servlet.xml:359`) | 50-51 | GAP-012, GAP-014 |
...
| Iteration metrics | The metrics repository is null, so the developer figures are expected to be empty (AF-01; class `ViewIterationMetricsAction#getRepository`) | 107-108 | GAP-014 |
| Task re-estimate page | Mapped to a plain Struts `Action` (`WAR:WEB-INF/action-servlet.xml:298`) | 161 | none |
| Print layout | `tiles:print` is selected by a `print` request parameter on every `xplanner:content` page; no page links to it (`ContentTag#doStartTag`, `WAR:WEB-INF/tiles-definitions.xml:23-28`) | 192 | none |
| iCal feed | Both queries use `task.story`, which `Task` does not map; an error response is expected (class `iCalServlet#generateTimeEntryData`, `#generateTaskData`, `#doGet`) | 228 | GAP-014 |
| SOAP `getCurrentIteration` and `deleteAttribute` | Undeclared alias `object` and missing `projectId` (`XPlanner#getCurrentIteration`); `a.targetId`/`a.name` on an entity with an embedded id (`AttributeRepositoryImpl#delete`) | 220-221 | GAP-014 |
| jrpdf report data sources | `task.story` in `PdfReportExporter$UserStoryDataSource` and `$PersonDataSource`, in addition to the missing templates | 211, 213 | GAP-003, GAP-014 |
| Duplicate Spring context | `spring-beans.xml` is loaded by the root context and by the Struts plug-in context (`WAR:WEB-INF/web.xml:32-38`, `WAR:WEB-INF/struts-config.xml:438-441`); duplicate reminder e-mails and Liquibase run expected | 65, 200-201 | GAP-014 |
| Delete person, change locale, formatting help | Unlinked route, or a hard-coded `/xplanner` link | 49, 58, 190 | GAP-008 |
...
|---|---|---|
| Remember-me stores user ID and password in Base64 cookies; the box is checked by default. The cookies have max-age 2147483647 seconds (about 68 years), no Secure flag and no path; the story-import settings cookies use the same helper (BA-001-07, pass-005 F-003) | `WAR:WEB-INF/jsp/security/login.jsp:29`; class `com.technoetic.xplanner.security.CredentialCookie#set`; class `CookieSupport#createCookie` | 15-16, 119 |
| Passwords are stored as salted MD5 digests | class `XPlannerLoginModule#digestPassword` (string "MD5") | 11 |
| No security filter on `/rest/*`, including task status update | `WAR:WEB-INF/web.xml:130-181` (no mapping for `/rest/*`) | 223-226 |
| The SOAP servlet is also mapped at `/servlet/AxisServlet`, outside the Basic filter | `WAR:WEB-INF/web.xml:298-301` | 216 |
| The Axis global configuration contains a default `adminPassword` parameter | `WAR:WEB-INF/server-config.wsdd:4` | 215 |
...
| The task time-log query is built by concatenating the `oid` request parameter into an HQL `where` clause | `WAR:WEB-INF/jsp/view/task.jsp:102` | 143 |
| The note list on project, iteration, story and task pages concatenates the `oid` request parameter into its HQL `where` clause (BA-001-07, mechanism (b)) | `WAR:WEB-INF/jsp/view/notes.jsp:13-15`; class `UseBeansTag#doEndTag` | 169 |
| The aggregate timesheet appends the posted person ids unchanged into its HQL, with only the dates bound; the query text is kept in a static field (BA-001-07, pass-005 F-002; exploitability not established) | class `AggregateTimesheetQuery#getTimesheet`; class `AggregateTimesheetForm#setSelectedPeople` | 165 |
| Raw request parameters are written unescaped into pages: `${param.fkey}` into the task board script, `oid`, `returnto`, `fkey`, `projectId` and further parameters into 17 edit and confirmation pages, and `oid` into the not-found message (BA-001-07, pass-005 F-003 and mechanism (b)) | `WAR:WEB-INF/jsp/view/dashboard.jsp:132`; `WAR:WEB-INF/jsp/common/generalError.jsp:26`; per-page lines in `.migration-tmp/stage-01/out/reflections-07.txt` | 35, 43, 50, 57, 78, 88, 98, 102, 114, 115, 121, 129, 134, 144, 149, 161, 169, 194, 196 |
| The container error page (every `Throwable`) and the system-information page print the exception message unescaped and list the referer, URL, query string, every request parameter (including a password field of the failing request) and every request attribute; the listed values are HTML-encoded (BA-001-07, pass-005 F-003) | `WAR:WEB-INF/web.xml:16-29`; `WAR:WEB-INF/jsp/common/unexpectedError.jsp:66,100-121`; class `BoxedListTag` | 55-56 |
...
| Descriptor URL role constraints are never evaluated (F-001, F-004) | `SecurityConfiguration#isAuthorized` has no caller; `WAR:WEB-INF/mobile-security.xml:11-15` | 32, 233 |
| Personal timesheet and ID jump apply no read check | `ViewTimesheetAction`, `IdSearchAction` (no authorization call reachable) | 164, 184 |
| SOAP attribute operations have no permission check | `XPlanner#setAttribute`, `#getAttribute`, `#deleteAttribute`, `#getAttributes`, `#getAttributesWithPrefix` | 218 |
| The unauthenticated login page shows the factory default administrator login pair in the default-bundle help text; the pair matches the seeded system administrator. Bundles without the key fall back to that text; `--` and `de` omit it. The value is not reproduced (CHK-009). Whether earlier installations still accepted the pair is not established (BA-001-06, pass-004 F-001) | `WAR:WEB-INF/jsp/security/login.jsp:34-39`; `WAR:WEB-INF/classes/xplanner-custom.properties:49`; `WAR:WEB-INF/classes/ResourceBundle.properties:784`; `WAR:WEB-INF/classes/db-changelog.xml:403-404`; class `XPlannerLoginModule#isPasswordMatched` | 9, 64 |
| The live activity log (user ID, client IP, request URI and query string of every `/do/*` request) is written into the public web root, where no filter applies (BA-001-06, pass-004 F-005) | `WAR:WEB-INF/classes/log4j-war.xml:44-58`; `WAR:WEB-INF/web.xml:45-52,155-176`; class `ActivityLogFilterHelper` | 73 |
| `index.jsp` has no security filter; without login it queries the visible projects and redirects to the current iteration, revealing its ID (BA-001-06, pass-004 F-005) | `WAR:WEB-INF/web.xml:155-176,321-323`; `WAR:index.jsp:7-31` | 18 |
...
**In scope for [`analysis/legacy_user_flows.xlsx`](./legacy_user_flows.xlsx)** (210 atomic rows in 18 epics, ordered entry capabilities, then core domain, then supporting features, then interop):
...
- Row statuses, counted from the workbook after BA-001-07: 111 `Yes`, 77 `Inferred`, 21 `Partial`, 1 `No` (210 in total).
- BA-001-07 added no rows and changed 31; see [`analysis/stages/stage-01/stage-02-pass-005-dispositions.md`](./stages/stage-01/stage-02-pass-005-dispositions.md).
- BA-001-06 added 2 rows, changed the content of 11 and renumbered references in 13; see [`analysis/stages/stage-01/stage-02-pass-004-dispositions.md`](./stages/stage-01/stage-02-pass-004-dispositions.md).
...
- 68 of the 73 JSPs are cited by name (recounted in BA-001-04, BA-001-05 and BA-001-06; `layout/viewLayoutNew.jsp` is cited by row 193).
...
- Unused libraries and assets: p6spy, `flash/Dashboard.swf`, `js/chart.html`, and the `printLink` tag, which no JSP uses. The `tiles:print` definition is used and is covered by row 192 (BA-001-06).
...
|---|---|---|---|---|
| [`analysis/reviews/stage-02-pass-001.md`](./reviews/stage-02-pass-001.md) F-001 (high) | accepted, mechanism narrowed: no server-side check on web view/edit/delete and most actions; DispatchForward does check when its default flag is on | Runnable Surfaces enforcement table; GAP-007; Q3 facts; rows 26, 28-32, 164, 184, 218, 225, 233-234 | `authtrace.js`, `disasm.js`; Stage 3 confirmation open | pending: next fresh Stage 2 pass |
| [`analysis/reviews/stage-02-pass-001.md`](./reviews/stage-02-pass-001.md) F-002 (medium) | accepted | row 186 rewritten, row 187 added | `authtrace.json` history sinks | pending |
| [`analysis/reviews/stage-02-pass-001.md`](./reviews/stage-02-pass-001.md) F-003 (low) | accepted | rows 69-71 (old row 63 split) | all 10 bundles compared | pending |
| [`analysis/reviews/stage-02-pass-001.md`](./reviews/stage-02-pass-001.md) F-004 (low) | accepted | Source Inventory security configuration row; rows 32, 233 | caller search with positive control | pending |
...
| [`analysis/reviews/stage-02-pass-001.md`](./reviews/stage-02-pass-001.md) F-006 (low) | accepted | Data And Integrations; GAP-008; row 202 | class symbols | pending |
| [`analysis/reviews/stage-02-pass-001.md`](./reviews/stage-02-pass-001.md) F-007 (medium) | accepted | rows 167, 180 added; related rows 164, 184 added | `authtrace.json`; class symbols | pending |
| [`analysis/reviews/stage-02-pass-001.md`](./reviews/stage-02-pass-001.md) F-008 (low) | accepted | Data And Integrations file storage; row 172 (`Inferred` to `Yes`) | class symbols; `db-changelog.xml:234` | pending |
| [`analysis/reviews/stage-02-pass-002.md`](./reviews/stage-02-pass-002.md) F-001 (medium) | accepted | rows 89, 152 corrected; rows 153-160 added (8 time-entry validation conditions); GAP-009 | `TimeEditorForm#valideRow` constants; bundle keys at `ResourceBundle.properties:265,675-684`; form-key sweep of all 27 form classes | pending: next fresh Stage 2 pass |
| [`analysis/reviews/stage-02-pass-002.md`](./reviews/stage-02-pass-002.md) F-002 (low) | accepted, extended (person delete, note delete) | rows 82, 91, 126, 142 (cascades); rows 49, 174 | `@OneToMany`/`@OneToOne` cascade annotations; `db-changelog.xml:258-267` foreign keys; `NoteHelper#deleteNote` | pending: next fresh Stage 2 pass |
| [`analysis/reviews/stage-02-pass-002.md`](./reviews/stage-02-pass-002.md) F-003 (low) | accepted | row 193; Data And Integrations social row | `viewLayout.jsp:73-88,92-95`; `viewLayoutNew.jsp:114-117` | pending: next fresh Stage 2 pass |
| [`analysis/reviews/stage-02-pass-002.md`](./reviews/stage-02-pass-002.md) F-004 (low) | accepted | row 171; Parity-Map Boundary first-pass list | `Note#getAttachmentCount`; `notes.jsp:80-92`; bundle label per variant | pending: next fresh Stage 2 pass |
| [`analysis/reviews/stage-02-pass-002.md`](./reviews/stage-02-pass-002.md) F-005 (low) | accepted | rows 14, 98 | `AuthenticationAction#execute` bytecode listing; `editIterationStatus.jsp:43-47,58-60` | pending: next fresh Stage 2 pass |
| [`analysis/reviews/stage-02-pass-002.md`](./reviews/stage-02-pass-002.md) F-006 (low) | accepted | row 76 | `projects.jsp:65-75,82-84` | pending: next fresh Stage 2 pass |
...
| [`analysis/reviews/stage-02-pass-002.md`](./reviews/stage-02-pass-002.md) F-008 (low) | accepted | row 117 (`Partial` to `Yes`); row 119 added; Source Inventory absent-resources row; GAP-003; Data And Integrations | `ImportStoriesAction#execute` exception table; `#setCookies`, `#populateForm` | pending: next fresh Stage 2 pass |
| [`analysis/reviews/stage-02-pass-002.md`](./reviews/stage-02-pass-002.md) F-009 (low) | accepted, mechanism narrowed (stored label follows the server default locale) | row 135 (`Yes` to `Inferred`), row 228; GAP-011 | `out/task-type-labels.json` (all 10 bundles); `editTask.jsp:52-69`; `iCalServlet#generateTaskData` | pending: next fresh Stage 2 pass |
...
| [`analysis/reviews/stage-02-pass-003.md`](./reviews/stage-02-pass-003.md) F-001 (medium) | accepted, extended (SOAP `deleteAttribute`, jrpdf data sources, undeclared alias in `getCurrentIteration`) | rows 11, 196, 211, 213, 220, 221, 228 (`Yes` to `Partial`); Source Inventory persistence row; Runnable Surfaces; GAP-011, GAP-014; Q2 facts | `hql-sweep-05.js` over 105 query sources; `iCalServlet` and `XPlanner` bytecode listings | pending: next fresh Stage 2 pass |
| [`analysis/reviews/stage-02-pass-003.md`](./reviews/stage-02-pass-003.md) F-002 (low) | accepted, extended (third loader without caller; Tiles read by two containers) | row 65 added; rows 64, 200, 201; Source Inventory Spring row; Runnable Surfaces job and bootstrap rows; Data And Integrations SMTP row; GAP-014; Q2 facts | `loaders-05.js` over 55 configuration files; `web.xml:32-38,194-196,227-249`, `struts-config.xml:438-441` | pending: next fresh Stage 2 pass |
| [`analysis/reviews/stage-02-pass-003.md`](./reviews/stage-02-pass-003.md) F-003 (low) | accepted, extended (merge branch of `MoveContinueTaskForm`, merge path of `TaskEditorForm`) | rows 52, 124, 132, 147 added; rows 131, 137, 146 | `chk005-validators-05.js` (17 validators, 47 keys, positive control) | pending: next fresh Stage 2 pass |
| [`analysis/reviews/stage-02-pass-003.md`](./reviews/stage-02-pass-003.md) F-004 (low) | accepted | row 110 added; rows 109, 111, 112; Source Inventory Charts row | `iterationStatistics.jsp:155-191`, `xplanner.properties:66-68`; `jsp-charts-includes-05.js` | pending: next fresh Stage 2 pass |
| [`analysis/reviews/stage-02-pass-004.md`](./reviews/stage-02-pass-004.md) F-001 (medium) | accepted | rows 9, 11, 64; rows 8, 12, 13, 56 (per-bundle notes); GAP-007; Q3 facts; unauthenticated-surfaces list | `login.jsp:34-39`, `ResourceBundle.properties:784`, `db-changelog.xml:403-404`; digest checked in memory, value not reproduced (CHK-009) | pending: next fresh Stage 2 pass |
| [`analysis/reviews/stage-02-pass-004.md`](./reviews/stage-02-pass-004.md) F-002 (low) | accepted | row 114 (`Partial` to `Yes`, column names corrected); Q2 facts row removed | `LinkTag#addNavigationParameters` listing; `globalLinks.jsp:14-18`; `dashboard.jsp:101-103,131-132` | pending: next fresh Stage 2 pass |
| [`analysis/reviews/stage-02-pass-004.md`](./reviews/stage-02-pass-004.md) F-003 (low) | accepted, extended (pool settings `hibernate.dbcp.*` also not delivered; 42 keys without a consumer) | row 68 added; Source Inventory Configuration row; P-01 | `spring-beans.xml:60-69,77-81`; `config-consumers-06.js` | pending: next fresh Stage 2 pass |
| [`analysis/reviews/stage-02-pass-004.md`](./reviews/stage-02-pass-004.md) F-004 (low) | accepted | row 192 added; Tiles row; Q2 facts; Parity-Map Boundary exclusions | `ContentTag#doStartTag`, `PrintLinkTag#isInPrintMode` listings; `tiles-definitions.xml:23-28` | pending: next fresh Stage 2 pass |
| [`analysis/reviews/stage-02-pass-004.md`](./reviews/stage-02-pass-004.md) F-005 (low) | accepted, extended (`/cewolf/*` unfiltered) | rows 18, 73; GAP-007, GAP-013; Q3 facts; Runnable Surfaces | `unauth-06.js`; `log4j-war.xml:44-58`; `web.xml:45-52,155-176,321-323` | pending: next fresh Stage 2 pass |
| [`analysis/reviews/stage-02-pass-004.md`](./reviews/stage-02-pass-004.md) F-006 (low) | accepted | row 189; Data And Integrations wiki row | `GenericWikiAdapter#isTopicExisting` listing with exception table; `xplanner.properties:113-116`; `render-side-effects-06.js` | pending: next fresh Stage 2 pass |
| [`analysis/reviews/stage-02-pass-005.md`](./reviews/stage-02-pass-005.md) F-001 (medium) | accepted | rows 50-51 (`Yes` to `Partial`); dormant-code row; GAP-012; Q2 facts | `action-servlet.xml:2,359`; `spring-dao.xml:17-57`; `ImportPeopleAction#execute` exception table; `wiring-07.js` | pending: next fresh Stage 2 pass |
| [`analysis/reviews/stage-02-pass-005.md`](./reviews/stage-02-pass-005.md) F-002 (medium) | accepted | row 165; GAP-007; Q3 facts | `AggregateTimesheetQuery#getTimesheet` listing; `query-sinks-07.js` | pending: next fresh Stage 2 pass |
| [`analysis/reviews/stage-02-pass-005.md`](./reviews/stage-02-pass-005.md) F-003 (low) | accepted; narrowed for the request listing, whose values are HTML-encoded | rows 15, 55, 56, 114, 119; GAP-007; Q3 facts | `dashboard.jsp:132`; `unexpectedError.jsp:66,100-121`; `BoxedListTag`; `CookieSupport#createCookie` | pending: next fresh Stage 2 pass |
| [`analysis/reviews/stage-02-pass-005.md`](./reviews/stage-02-pass-005.md) F-004 (low) | accepted | rows 232, 234; Mobile channel row; Runnable Surfaces; Q2 facts; GAP-014 | `mobile-struts-config.xml:20-49`; `wiring-impact-07.js` | pending: next fresh Stage 2 pass |
| [`analysis/reviews/stage-02-pass-005.md`](./reviews/stage-02-pass-005.md) F-005 (low) | accepted, narrowed (the listing request fails before the write) | rows 172, 176; Data And Integrations file storage row; Q2 facts | `FileSystemImpl#getRootDirectory`, `#getDirectory`; `FileManagerAction#doExecute`; `persist-trace-07.js` | pending: next fresh Stage 2 pass |
| [`analysis/reviews/stage-02-pass-005.md`](./reviews/stage-02-pass-005.md) mechanisms (a)-(c) | related occurrences corrected | rows 35, 43, 57, 78, 88, 98, 102, 115, 121, 129, 134, 144, 149, 161, 169, 176, 192, 194, 196, 205; GAP-007; Q3 facts | the inventories above | pending: next fresh Stage 2 pass |

The pass-005 correction is recorded in [`analysis/stages/stage-01/stage-02-pass-005-dispositions.md`](./stages/stage-01/stage-02-pass-005-dispositions.md). That record holds the PM assignment, the scope validation, the three mechanism inventories and the retained areas with their absence-of-impact reasons. The pass-004 per-finding details are in [`analysis/stages/stage-01/stage-02-pass-004-dispositions.md`](./stages/stage-01/stage-02-pass-004-dispositions.md). They include the link-parameter, unauthenticated-surface, configuration-consumer and rendering side-effect sweeps, the CHK-009 credential scan and the source checks for the reviewer proposals P-1 to P-4. The pass-003 per-finding details are in [`analysis/stages/stage-01/stage-02-pass-003-dispositions.md`](./stages/stage-01/stage-02-pass-003-dispositions.md). They include the query, loader, validator and chart sweeps and the source checks for the reviewer proposals P-1 and P-2. The pass-002 per-finding details are in [`analysis/stages/stage-01/stage-02-pass-002-dispositions.md`](./stages/stage-01/stage-02-pass-002-dispositions.md). They include the CHK-002, CHK-003 and CHK-004 sweep row lists and the source checks for the reviewer proposals P-1 to P-3. The pass-001 details are in [`analysis/stages/stage-01/stage-02-pass-001-dispositions.md`](./stages/stage-01/stage-02-pass-001-dispositions.md). They include the source evidence, checks performed, remaining work, the sweep findings AF-01 and AF-02, and the reviewer checklist proposals.
...
- [x] The parity workbook is populated and linked to concrete evidence (210 rows after BA-001-07).
...
- **Learning update, BA-001-07:** source verification confirms the errors behind the pass-005 reviewer proposals P-1 (every injected property needs a wiring bean) and P-2 (trace request-derived values to HQL/SQL, unescaped output, error pages and cookies); see the correction record.
- **Self-check, BA-001-07 (current):** impact-scoped Stage 1 correction after [`analysis/reviews/stage-02-pass-005.md`](./reviews/stage-02-pass-005.md), within the boundary in [`analysis/stages/stage-01/stage-02-pass-005-dispositions.md`](./stages/stage-01/stage-02-pass-005-dispositions.md). The scope is the rows, sections and mechanism occurrences listed there.
  - Checklist [`analysis/error-prevention-checklist.md`](./error-prevention-checklist.md) SHA-256 `7a5683a8e0411f7d748ffc873fae2ec590ce0138a77a313fb92df4837553694d`. Applicable checks: **CHK-001..CHK-011**. Pass 005 linked F-001, F-004 and F-005 to CHK-003.
  - **CHK-001:** `.migration-tmp/stage-01/tools/chk001-ba-001-07.js` covered every new or changed citation.
  - **CHK-003:** the wiring, request-value and read-path inventories were rerun. The CHK-003 applicability now includes injected properties.
  - **CHK-007:** figures were regenerated: 210 rows, 111 `Yes`, 77 `Inferred`, 21 `Partial`, 1 `No`.
  - **CHK-009:** the credential scan result is in RESULT BA-001-07.
  - **Other checks:** the remaining checks are either unaffected, or retained from BA-001-06 as described in the correction record.
- **Self-check, BA-001-06 (historical):** Stage 1 re-entry after [`analysis/reviews/stage-02-pass-004.md`](./reviews/stage-02-pass-004.md). The scope is both Stage 1 records and the correction record [`analysis/stages/stage-01/stage-02-pass-004-dispositions.md`](./stages/stage-01/stage-02-pass-004-dispositions.md), at the versions given in RESULT BA-001-06.
  - Checklist [`analysis/error-prevention-checklist.md`](./error-prevention-checklist.md) SHA-256 `0c2350b24a1610a3b6eae768aad6b70a8eaabc7fd19d04ca83c7455d39f9df84`. Applicable checks: **CHK-001..CHK-009**. Pass 004 linked F-001 to CHK-004 and F-002 and F-003 to CHK-003.
  - **CHK-001:** `.migration-tmp/stage-01/tools/chk001-ba-001-06.js` checked every new or changed line citation and the row references used in BA-001-06 text. The final run has 0 failures; the counts are in RESULT BA-001-06.
  - **CHK-002:** rows 18 and 73 now state their unauthenticated access condition. No other row changed a permission condition.
  - **CHK-003:** the configuration keys were swept against their consumers (row 68). The link parameters added by `LinkTag` were traced (row 114). The rendering side effects are recorded (rows 189, 192).
  - **CHK-004:** the texts of all public pages were compared in all 10 bundles (rows 8, 9, 12, 13, 56).
  - **CHK-005 to CHK-008:** no validator, delete or query statement changed. Figures were regenerated: 210 rows, 113 `Yes`, 77 `Inferred`, 19 `Partial`, 1 `No`, 68 of 73 JSPs, all 87 action paths.
  - **CHK-009:** no credential value appears in this record, the workbook, the correction record or the new scratch outputs. `chk009-scan-06.js` found 0 hits; the result is in RESULT BA-001-06.
- **Learning update, BA-001-06 (historical):** source verification confirms the errors behind the reviewer proposals P-1 (tag-added link parameters), P-2 (mechanical list of unauthenticated surfaces), P-3 (a configuration value is effective only when its consumer receives it) and P-4 (rendering-path side effects). BA supports all four; PM decides admission and whether P-3 and P-4 refine CHK-003.
- **Self-check, BA-001-05 (historical):** Stage 1 re-entry after [`analysis/reviews/stage-02-pass-003.md`](./reviews/stage-02-pass-003.md). The scope is both Stage 1 records and the correction record [`analysis/stages/stage-01/stage-02-pass-003-dispositions.md`](./stages/stage-01/stage-02-pass-003-dispositions.md), at the versions given in RESULT BA-001-05.
  ...
  - **CHK-002:** the changed rows state no new permission condition. Row 221 keeps its server-side statement.
  ...
  - **CHK-004:** no locale-dependent statement changed. The CHK-004 notes that point to the date rows now cite rows 69-71.
  - **CHK-005:** all 17 validators and 47 keys are covered, with a positive control.
  - **CHK-006:** the delete rows are unchanged. SOAP `deleteAttribute` (row 221) is recorded as a failing query, not as a delete effect.
...
- **Self-check, BA-001-04 (historical):** Stage 1 re-entry after [`analysis/reviews/stage-02-pass-002.md`](./reviews/stage-02-pass-002.md). The scope is both Stage 1 records and the correction record [`analysis/stages/stage-01/stage-02-pass-002-dispositions.md`](./stages/stage-01/stage-02-pass-002-dispositions.md), at the versions given in RESULT BA-001-04.
  ...
  - **CHK-003:** applied to every row that attributes an effect, hook, helper, decorator or cascade. Rows 14, 76 and 117 were corrected, and cascade and cookie paths were added. The row list is in the correction record.
  - **CHK-004:** applied to every locale-dependent row. Rows 135 and 171 were corrected, and per-bundle notes were added to rows 14, 89, 98, 152-160, 163, 166 and 228.
