...
> [!WARNING]
> **Stage 1 re-entry draft (BA-001-03, after Stage 2 pass 001 `findings`): corrected static analysis of the fixed baseline package, not yet independently re-verified**
>
> The whole WAR was enumerated (964 files). Stage 2 pass 001 returned 8 findings. All 8 were checked against the source: 7 accepted and F-001 accepted with a narrowed mechanism. The main correction is that generic web view, edit and delete actions have no server-side permission check (F-001, recorded as risk under GAP-007/Q3). The parity map now holds 193 scenario rows in 18 epics: 112 `Yes`, 60 `Inferred`, 20 `Partial` and 1 `No`; 14 rows were added and 10 changed. Dispositions are in [`analysis/stages/stage-01/stage-02-pass-001-dispositions.md`](./stages/stage-01/stage-02-pass-001-dispositions.md). All rows are static-only, and behavior known only from compiled classes stays `Inferred` because no JDK or decompiler was available. The package is the project's fixed baseline, not an unmodified original. Three ZIP entries carrying the known MySQL patch show a rewrite sign, and 31 rows carry per-conclusion provenance notes (GAP-005, Q1 decided by the owner). Fourteen gaps are recorded (GAP-001..GAP-014). Q2-Q4 are deferred by the owner to Stages 4 and 9. Insecure legacy behavior (GAP-007) is not permission to carry it into the new application.
>
> **Next:** PM verifies RESULT BA-001-03, integrates status and launches a new fresh blind Stage 2 pass. The corrections are not yet independently verified. Decisions and open questions are in [Owner Decisions And Open Questions](#read-owner-decisions-and-open-questions).
>
...
- Snapshot date: WAR entry timestamps run from 2011-11-20 to 2011-12-04. The analysis was performed on 2026-09-23. The bounded correction BA-001-02 and the re-entry correction BA-001-03 were made on 2026-09-24.
- Analyst: Business Analyst (role `ba`, mode `author`), tasks **BA-001-01** (first pass), **BA-001-02** (bounded correction applying the owner's Q1 decision) and **BA-001-03** (Stage 1 re-entry after [`analysis/reviews/stage-02-pass-001.md`](./reviews/stage-02-pass-001.md)), all assigned by PM (task PM-001, Claude Code session `d0ec1166-ffc9-446a-ba86-d768242e6de8`).
...
| Other web actions (export, import, move/continue, reorder, time, notes, attachments, file manager, integration queue, iteration start/close/continue, notification receivers, role editor, timesheet, ID search, test/admin utilities, "me" page) | none as a gate | `out/authtrace.json` | 30, 35, 148, 168 |
| Person editor role changes | only for projects where the user holds `admin.edit.role` | `EditPersonHelper#modifyRoles/#isCurrentUserAdminOfProject` | 31 |
| DispatchForward views with authorization required (default true): `/do/view/integrations` and the 7 WAP actions | projectId parameter and `system.project` read, else not-authorized forward | `DispatchForward#<init>/#isSecure/#execute` | 28, 217 |
| Content search, aggregate timesheet | result filtering by read permission | `SearchResultAuthorizationPredicate#isResultReadableByUser`, `AggregateTimesheetQuery#getTimesheet` | 151, 164 |
| SOAP | per-object read, create, edit and delete checks; the 5 attribute operations are unchecked | `XPlanner#hasPermission` via `#getObject`, `#selectAccessibleObjects`, `#addObject`, `#updateObject`, `#removeObject` | 201 |
| REST | none, and no filter | `ViewService`, `UpdateService` | 208 |
| iCal | another person's calendar needs `admin.edit` on `system.person` | `iCalServlet#doGet` | 213 |
| Descriptor URL role constraints | never evaluated | `SecurityConfiguration#isAuthorized` has no caller | 32, 216 |
...
| Twitter and Facebook | Share links and the like widget (`connect.facebook.net` script) on view pages | `WAR:WEB-INF/jsp/layout/viewLayout.jsp:73-95` | not checked | Browser-side call to external services. |
...
| Excel spreadsheets and comma-separated text | Story import (POI) and people import (text lines) | `SpreadsheetStoryImporter`, `ImportPeopleAction` | not checked | The people template is `.xls` but the parser reads comma-separated lines (GAP-012). |
...
|---|---|---|---|---|
| `sha256sum legacy/*` (before the work and again at the end) | revision `1e12347`, legacy tree `3bd350fa` | passed: all four hashes equal the values in [Scope And Provenance](#read-scope-and-provenance) | command output in the task transcript; re-check command recorded here | Nothing about behavior. |
...
| `node .migration-tmp/stage-01/tools/build-workbook.js` | exceljs from the project-local analysis tool dependencies; governed `setFill`, `reconcileWorksheetOutlineHierarchy`, `writeWorkbookFile` | passed: 18 epics, 179 scenario rows, outline reconciled, header rows 1-6 unchanged | [`analysis/legacy_user_flows.xlsx`](./legacy_user_flows.xlsx) | Writing the rows does not prove them correct. Stage 2 must verify them independently. |
| `npm --prefix analysis/tools run sync:workbook-progress` | same | passed: 18 epic banners updated to `(0%)`; overall 0 / 179 | workbook row 3 and banners | Progress is 0% by definition at Stage 1. |
| `npm --prefix analysis/tools run audit:workbook` | same | passed: `WORKBOOK AUDIT OK` (Scenarios 179; open 179; epics 18; revision sheets 0) | audit output | Structure and colour rules only, not semantic correctness. |
| `npm --prefix analysis/tools run audit:project` | same | passed (exit 0) | audit output | Project configuration structure only. |
| `npm --prefix analysis/tools run audit:artifact-links` | same | passed (exit 0) | audit output | Link form only. |
| `node analysis/tools/artifact-reading.js --file analysis/legacy_reconnaissance.md` | same | passed: no errors | tool output | Reading structure only. |
| `npm --prefix analysis/tools run audit:workbook:excel` | Excel Desktop (Office16) through COM, read-only open, run once | passed (exit 0): `EXCEL DESKTOP OPEN AUDIT OK`; workbook hash unchanged | `.migration-tmp/stage-01/out/audit-excel.txt` | A clean open does not prove content or visual layout. |
...
|---|---|---|---|---|---|
| GAP-001 | No JDK, javap or decompiler: method bodies, branch conditions, default values and error paths in 594 classes are unread. | 48 parity rows are `Inferred`, and parts of many `Yes` rows rest on class symbols. | Stage 3 live walkthrough, or an owner decision to permit a decompiler inside the project boundary. | PM, then the owner | open. Scratch class reader limits are stated in [Build, Run, And Test Evidence](#read-build-run-and-test-evidence). |
...
| GAP-003 | Referenced resources are missing from the WAR: 7 JSPs (`security/notAuthorized.jsp`, `edit/editSetting.jsp`, `admin/invalidateCache.jsp`, `view/iterationIterations.jsp`, `view/iterationTabs.jsp`, `view/taskhistory.jsp`, `view/viewtext.jsp`), 3 classes (`net.sf.xplanner.domain.Feature`, `...importer.MissingWorksheetException`, `net.sf.xplanner.soap.XPlanner`), 4 JasperReports templates (`JRIteration`, `JRStory`, `JRTask`, `JRPerson` `.jrxml`). | The affected flows are marked `Partial`: not-authorized page, settings editor, admin cache page, tabbed iteration view, features, worksheet-not-found import error, jrpdf reports. | Confirm at Stage 3. Stage 4 owner decides parity for broken legacy features. | BA (Stage 3), owner (Stage 4) | open. `out/jsp-refs.txt`, descriptor reference check, JAR entry search. |
...
| GAP-011 | The 102 library JARs were not analysed beyond entry names. Framework behavior (Struts, Spring, Hibernate, Axis, Jersey, displaytag) is assumed standard for the bundled versions. | Framework defaults such as validation, paging and error handling are inferred. | Stage 3 observation. | BA | open |
...
|---|---|---|---|
| Story features (view, edit, delete; project and iteration lists) | Action type `net.sf.xplanner.domain.Feature` is absent; `com.technoetic.xplanner.domain.Feature` is unmapped; links are commented out (`WAR:WEB-INF/action-servlet.xml:96-100,202-212`; `WAR:WEB-INF/jsp/view/iteration/links.jsp:32-35`) | 177 | GAP-003, GAP-004 |
| Continuous integration queue | Entity class unmapped; no live link (`WAR:WEB-INF/struts-config.xml:388-394`) | 179-181 | GAP-004 |
| Settings pages | Editor JSP absent; list page unlinked (`WAR:WEB-INF/struts-config.xml:405,419-421`) | 58-59 | GAP-003 |
| JasperReports (jrpdf) exports | Templates `JR*.jrxml` absent (class `PdfReportExporter`) | 194, 196 | GAP-003 |
...
| Mobile WAP pages | Links hard-code `/xplanner`; web filter may intercept mobile login; DispatchForward requires a projectId the links do not pass (`WAR:WEB-INF/mobile-struts-config.xml:20-49`) | 215-217 | GAP-008, GAP-014 |
| JAX-WS endpoint | Servlet commented out, class absent (`WAR:WEB-INF/sun-jaxws.xml:3`) | 202 | GAP-003 |
| File manager | Reachable only by URL (`WAR:WEB-INF/struts-config.xml:401-403`) | 160 | none |
...
| Task re-estimate page | Mapped to a plain Struts `Action` (`WAR:WEB-INF/action-servlet.xml:298`) | 145 | none |
| Iteration task board | Parameter mismatch between link and REST call (`WAR:WEB-INF/jsp/view/dashboard.jsp:132`) | 110 | none |
| Delete person, change locale, formatting help | Unlinked route, or a hard-coded `/xplanner` link | 49, 57, 174 | GAP-008 |
...
| No security filter on `/rest/*`, including task status update | `WAR:WEB-INF/web.xml:130-181` (no mapping for `/rest/*`) | 206-209 |
| The SOAP servlet is also mapped at `/servlet/AxisServlet`, outside the Basic filter | `WAR:WEB-INF/web.xml:298-301` | 199 |
| The Axis global configuration contains a default `adminPassword` parameter | `WAR:WEB-INF/server-config.wsdd:4` | 198 |
...
| The task time-log query is built by concatenating the `oid` request parameter into an HQL `where` clause | `WAR:WEB-INF/jsp/view/task.jsp:102` | 136 |
...
| Descriptor URL role constraints are never evaluated (F-001, F-004) | `SecurityConfiguration#isAuthorized` has no caller; `WAR:WEB-INF/mobile-security.xml:11-15` | 32, 216 |
| Personal timesheet and ID jump apply no read check | `ViewTimesheetAction`, `IdSearchAction` (no authorization call reachable) | 148, 168 |
| SOAP attribute operations have no permission check | `XPlanner#setAttribute`, `#getAttribute`, `#deleteAttribute`, `#getAttributes`, `#getAttributesWithPrefix` | 201 |
...
**In scope for [`analysis/legacy_user_flows.xlsx`](./legacy_user_flows.xlsx)** (193 atomic rows in 18 epics, ordered entry capabilities, then core domain, then supporting features, then interop):
...
- Row statuses: 112 `Yes`, 60 `Inferred`, 20 `Partial`, 1 `No` (BA-001-03: 14 rows added and 10 changed; see [`analysis/stages/stage-01/stage-02-pass-001-dispositions.md`](./stages/stage-01/stage-02-pass-001-dispositions.md)).
...
- 67 of the 73 JSPs are cited by name.
- The 6 JSPs not cited are:
  - five layout fragments with no behavior of their own: `common/header.jsp`, `common/formattingHelpJS.jsp`, `layout/viewHeader.jsp`, `layout/viewFooter.jsp`, `layout/viewLayoutNew.jsp`;
...
- task type labels;
- the attachment download counter;
...
|---|---|---|---|---|
| [`analysis/reviews/stage-02-pass-001.md`](./reviews/stage-02-pass-001.md) F-001 (high) | accepted, mechanism narrowed: no server-side check on web view/edit/delete and most actions; DispatchForward does check when its default flag is on | Runnable Surfaces enforcement table; GAP-007; Q3 facts; rows 26, 28-32, 148, 168, 201, 208, 216-217 | `authtrace.js`, `disasm.js`; Stage 3 confirmation open | pending: next fresh Stage 2 pass |
| [`analysis/reviews/stage-02-pass-001.md`](./reviews/stage-02-pass-001.md) F-002 (medium) | accepted | row 170 rewritten, row 171 added | `authtrace.json` history sinks | pending |
...
| [`analysis/reviews/stage-02-pass-001.md`](./reviews/stage-02-pass-001.md) F-004 (low) | accepted | Source Inventory security configuration row; rows 32, 216 | caller search with positive control | pending |
...
| [`analysis/reviews/stage-02-pass-001.md`](./reviews/stage-02-pass-001.md) F-006 (low) | accepted | Data And Integrations; GAP-008; row 185 | class symbols | pending |
| [`analysis/reviews/stage-02-pass-001.md`](./reviews/stage-02-pass-001.md) F-007 (medium) | accepted | rows 151, 164 added; related rows 148, 168 added | `authtrace.json`; class symbols | pending |
| [`analysis/reviews/stage-02-pass-001.md`](./reviews/stage-02-pass-001.md) F-008 (low) | accepted | Data And Integrations file storage; row 156 (`Inferred` to `Yes`) | class symbols; `db-changelog.xml:234` | pending |

The per-finding details are in [`analysis/stages/stage-01/stage-02-pass-001-dispositions.md`](./stages/stage-01/stage-02-pass-001-dispositions.md). They include the source evidence, checks performed, remaining work, the sweep findings AF-01 and AF-02, and the reviewer checklist proposals.
...
- [x] The parity workbook is populated and linked to concrete evidence (193 rows after BA-001-03).
- [x] Known exclusions and blockers are visible.
- [ ] Required deterministic audits pass. `audit:workbook`, `audit:project`, `audit:artifact-links` and `artifact-reading.js` pass, but the visual render pass is blocked (GAP-010), so this box stays unticked. This checklist is the author's draft self-assessment, not stage closure; closure needs a clean Stage 2 pass.
