...
> [!WARNING]
> **Stage 1 draft (after correction BA-001-02): static analysis of the project's fixed baseline package, nothing observed live**
>
> The whole WAR was enumerated (964 files). The parity map now holds 179 scenario rows in 18 epics. Of these, 112 are `Yes`, 48 `Inferred`, 18 `Partial` and 1 `No`. All rows are static-only, and behavior known only from compiled classes stays `Inferred` because no JDK or decompiler was available. The package is the project's fixed baseline, not an unmodified original. Three ZIP entries carrying the known MySQL patch show a rewrite sign, and 31 rows carry per-conclusion provenance notes (GAP-005, Q1 decided by the owner). Thirteen gaps are recorded (GAP-001..GAP-013). Q2-Q4 are deferred by the owner to Stages 4 and 9. Insecure legacy behavior (GAP-007) is not permission to carry it into the new application.
>
> **Next:** PM verifies RESULT BA-001-02, integrates status and launches the fresh blind Stage 2 reviewer. Decisions and open questions are in [Owner Decisions And Open Questions](#read-owner-decisions-and-open-questions).
>
...
- Snapshot date: WAR entry timestamps run from 2011-11-20 to 2011-12-04. The analysis was performed on 2026-09-23. The bounded correction BA-001-02 was made on 2026-09-24.
- Analyst: Business Analyst (role `ba`, mode `author`), tasks **BA-001-01** (first pass) and **BA-001-02** (bounded correction applying the owner's Q1 decision), both assigned by PM (task PM-001, Claude Code session `d0ec1166-ffc9-446a-ba86-d768242e6de8`).
...
| File storage for attachments | Note attachments and the file manager through the virtual file system (`xdir`/`xfile` tables) | `spring-beans.xml:263`, `EditNoteAction`, `FileManagerAction` | not checked | Whether file bytes are stored in the database or on disk was not determined (GAP-001). |
...
|---|---|---|---|
| Story features (view, edit, delete; project and iteration lists) | Action type `net.sf.xplanner.domain.Feature` is absent; `com.technoetic.xplanner.domain.Feature` is unmapped; links are commented out (`WAR:WEB-INF/action-servlet.xml:96-100,202-212`; `WAR:WEB-INF/jsp/view/iteration/links.jsp:32-35`) | 166 | GAP-003, GAP-004 |
| Continuous integration queue | Entity class unmapped; no live link (`WAR:WEB-INF/struts-config.xml:388-394`) | 168-170 | GAP-004 |
| Settings pages | Editor JSP absent; list page unlinked (`WAR:WEB-INF/struts-config.xml:405,419-421`) | 54-55 | GAP-003 |
| JasperReports (jrpdf) exports | Templates `JR*.jrxml` absent (class `PdfReportExporter`) | 182, 184 | GAP-003 |
| Tabbed iteration view | `view/iterationTabs.jsp` absent (`WAR:WEB-INF/struts-config.xml:199-201`) | 96 | GAP-003 |
| Not-authorized page | `security/notAuthorized.jsp` absent (`WAR:WEB-INF/struts-config.xml:139`) | 28 | GAP-003 |
| Mobile WAP pages | Links hard-code `/xplanner`; web filter may intercept mobile login (`WAR:WEB-INF/mobile-struts-config.xml:20-49`) | 202-203 | GAP-008 |
| JAX-WS endpoint | Servlet commented out, class absent (`WAR:WEB-INF/sun-jaxws.xml:3`) | 189 | GAP-003 |
| File manager | Reachable only by URL (`WAR:WEB-INF/struts-config.xml:401-403`) | 152 | none |
| Project role editor | Reachable only by URL (`WAR:WEB-INF/struts-config.xml:409-411`) | 31 | none |
| Task re-estimate page | Mapped to a plain Struts `Action` (`WAR:WEB-INF/action-servlet.xml:298`) | 139 | none |
| Iteration task board | Parameter mismatch between link and REST call (`WAR:WEB-INF/jsp/view/dashboard.jsp:132`) | 104 | none |
| Delete person, change locale, formatting help | Unlinked route, or a hard-coded `/xplanner` link | 45, 53, 163 | GAP-008 |
| Admin and test utilities in the production configuration | `WAR:WEB-INF/web.xml:233` loads `test-struts-config.xml` | 56-58 | GAP-007 |
...
| No security filter on `/rest/*`, including task status update | `WAR:WEB-INF/web.xml:130-181` (no mapping for `/rest/*`) | 193-196 |
| The SOAP servlet is also mapped at `/servlet/AxisServlet`, outside the Basic filter | `WAR:WEB-INF/web.xml:298-301` | 187 |
| The Axis global configuration contains a default `adminPassword` parameter | `WAR:WEB-INF/server-config.wsdd:4` | 186 |
| `/do/invalidateHibernateCache` bypasses login | `WAR:WEB-INF/security.xml:5` | 56 |
| Test and admin actions are loaded in production (clock shift, property edit, data sampling, notifier) | `WAR:WEB-INF/web.xml:233`; `WAR:WEB-INF/test-action-servlet.xml:4-20` | 56-58 |
| The task time-log query is built by concatenating the `oid` request parameter into an HQL `where` clause | `WAR:WEB-INF/jsp/view/task.jsp:102` | 130 |
| The system information page shows the database URL and user (password masked) | class `com.technoetic.xplanner.SystemInfo#getDatabaseInfo` | 50 |
| Database credentials are present in the shipped properties (values not reproduced, A3) | `WAR:WEB-INF/classes/xplanner.properties:14-15`, `xplanner-custom.properties:23-24` (P-01) | 61 |
...
- Workbook rows 46-48 are affected (GAP-012).

<a id="read-parity-map-boundary"></a>

## Parity-Map Boundary

**In scope for [`analysis/legacy_user_flows.xlsx`](./legacy_user_flows.xlsx)** (179 atomic rows in 18 epics, ordered entry capabilities, then core domain, then supporting features, then interop):
...
- Row statuses: 112 `Yes`, 48 `Inferred`, 18 `Partial`, 1 `No`.
...
- Unwired classes: `CsvExporter`, `TomcatUserImporter`, the autopatch launchers.
...
|---|---|---|---|---|
| not applicable: first entry | not applicable | not applicable | not applicable | pending: the first Stage 2 pass has not run |
...
- [x] The parity workbook is populated and linked to concrete evidence (179 rows).
...
- **Self-check, BA-001-02 (current):** Stage 1 first entry, bounded correction. The scope is both Stage 1 records at the result version given in RESULT BA-001-02.
