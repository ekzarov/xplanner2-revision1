...
const cheatNotes = {
  ...
  'stage-01': 'Read the immutable legacy source named by project.yaml. Populate the Bootstrap blanks; on return, verify triggering findings and investigate affected scope, not just the checklist.',
...
flowBlock('analysis/migration_artifact_flow.drawio', '<diagram id="stage-artifact-roles" name="Stage inputs, updates and review roles"><mxGraphModel page="1" pageWidth="1200" pageHeight="5400"><root><mxCell id="0"/><mxCell id="1" parent="0"/><mxCell id="stage-flow-legend" parent="1" vertex="1" value="' + escapeHtml(flowIntro) + '" style="text;whiteSpace=wrap;html=1;align=left;fontSize=15;"><mxGeometry x="40" y="20" width="1120" height="110" as="geometry"/></mxCell>' + flowCells + '</root></mxGraphModel></diagram>', (s,b) => s.replace('</mxfile>', b + '\n</mxfile>'));
require('../tools/sync-gate-review-guide').synchronize(targetRoot, check, allowedFiles);
