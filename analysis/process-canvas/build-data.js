...
const stages = [
  ...
  },
  {
    ...
    actions: ['Inventory channels, roles, routes, jobs and integrations', 'Trace behavior to source/configuration evidence', 'Record unknowns and deployment constraints', 'Create one atomic scenario per parity-map row'],
    ...
    reentry: {
      ...
      en: {
        ...
        steps: [
          'The primary agent reads the triggering record and earlier unresolved findings, then checks the legacy source, configuration and linked runtime evidence. Observed differences are investigated, not silently discarded.',
          'Correct the current map and reconnaissance; check related roles and mechanisms. Expand discovery if the baseline is unreliable, without automatically starting from zero.',
          'Record each finding ID, disposition, evidence, changed rows/files and remaining work. Update status and run the Stage 1 gates; the original review stays immutable.',
          'A fresh Stage 2 agent starts a new blind Phase A. Previous findings and correction records become available only in Phase B; fixes do not replace independent acceptance.',
        ],
      },
      ru: {
        ...
        steps: [
          'Основной агент читает запись возврата и прежние незакрытые замечания, затем проверяет исходники, конфигурацию и связанные доказательства из работающей системы. Наблюдаемые расхождения исследует, а не отбрасывает.',
          'Исправляет текущую карту и разведку, проверяет связанные роли и механизмы. Если исходная разведка ненадёжна, расширяет обследование; автоматического старта с нуля нет.',
          'Записывает ID замечания, принятое по нему решение, доказательства, изменённые строки/файлы и остаток работы. Обновляет статус и выполняет гейты шага 1; прежнее ревью не переписывает.',
          'Новый независимый агент шага 2 заново проводит слепую фазу A. Прежние замечания и исправления доступны только в фазе B; исправление ещё не означает независимую приёмку.',
        ],
      },
    },
    },
  {
    ...
    actions: ['Declare independence and the allowed Phase A inputs', 'Phase A: inspect source without the filled map, reconnaissance or prior conclusions', 'Save the independent inventory and its checkpoint before opening Phase B inputs', 'Phase B: check discovered behavior against records and every recorded claim back to source', 'Preserve Phase A; record matches, gaps, reviewer corrections and blocked scope in the final report'],
