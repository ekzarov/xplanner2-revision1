  ...
  },
  {
    ...
    reentry: {
      ...
      en: {
        ...
        steps: [
          'After the owner merges the triggering record PR, PM assigns a separate correction branch. Preserve in-flight work. The primary agent reads the triggering record and unresolved findings, then checks source, configuration and runtime evidence.',
          ...
          'Record finding IDs, dispositions, evidence, changed rows/files and remaining work. Update status and publish the correction PR; required gates/CI and owner merge apply. The original review stays immutable.',
          'After the correction PR is merged, a fresh Stage 2 agent starts a new blind Phase A on the integrated revision. Previous findings and correction records become available only in Phase B; fixes do not replace independent acceptance.',
        ],
      },
      ru: {
        ...
        steps: [
          'После merge владельцем PR с записью, вызвавшей возврат, PM выделяет отдельную ветку исправлений. Незавершённая работа сохраняется. Основной агент читает запись возврата и незакрытые замечания, затем проверяет исходники, конфигурацию и доказательства из работающей системы.',
          ...
          'Записывает ID замечаний, решения по ним, доказательства, изменённые строки/файлы и остаток работы. Обновляет статус и публикует PR исправлений; обязательны гейты/CI и merge владельца. Прежнее ревью не переписывает.',
          'После merge PR исправлений новый независимый агент шага 2 проводит слепую фазу A по интегрированной ревизии. Прежние замечания и исправления доступны только в фазе B; исправление ещё не означает независимую приёмку.',
        ],
      },
    },
    },
