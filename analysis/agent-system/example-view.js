...
const en={
  ...
  notes:[
    ...
    'Initial BA review 2 uses full-blind: save a complete neutral source-derived Phase A before opening the filled map, reconnaissance and conclusions for two-way Phase B. QA review 19 still saves independent live observations from neutral expectations before reconciliation. In these blind passes, withhold full status, learned checks, author handoffs and filled examples until Phase B; automatic memory or startup imports can invalidate blindness.',
    ...
    'This sequence uses four continuing specialist author sessions; QA needs no separate author session here. Support and parallel work are on demand, not a permanent team. It assumes no findings, repeated controls or additional support. On return, correct affected work and related mechanisms while retaining valid unaffected work. Wider authoring needs impact evidence and authority.',
    'Stage 2 correction loop (not illustrated): after a complete valid full-blind baseline, even with findings, a fresh independent read-only BA may perform bounded correction-validation, never an author or previous reviewer. Read earlier reports/checklist/dispositions immediately; pin root full report, snapshot/source hashes, predecessor, latest candidate and all intervening reports/changes. No new Phase A. Verify the entire diff, open findings, related mechanisms/dependencies and affected old matches; retain exact C IDs only with applicability rationale, without double counting. Clean requires whole-scope coverage and no unchecked/open items, including Low; CHK is not a ceiling. Bounded impact checks may expand. Changed source/channels/scope, contamination, unreliable/missing/incomplete baseline or systemic/unbounded impact stop closure as blocked/invalid and require a full new blind session before prior information. Stage 19 and other controls are unchanged. See analysis/reviews/README.md#stage-2-correction-validation.',
